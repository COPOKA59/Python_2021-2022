...
if not s:
    ...
    for i in range(2, n + 1):  # Перебор других множеств:
        m = int(s[i])  # Количество точек с координатами(X,Y) в множестве
        B = [0] * m * 2  # Массив для другого множества
        B = Koord(B, m)
        SRAV()

    nMax = max(C)  # Максимальное кол-во множеств у точки
    c = C.index(nMax)  # индекс точки с макс.множествами

    if nMax == 0:
        Fout.write('В файле точки первого множества не встречаются в других множествах')
    else:
        Fout.write("Точка ({}, {}) первого множества принадлежит ещё {} множеств(-у/-ам)".format(A[c*2], A[c*2+1], nMax))
