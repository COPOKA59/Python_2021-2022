...
class Tpeople:  # Класс человек
    ...
    def __getVM(self):
        return print(self.fio, self.__DR, self.pol)

    v0 = property(__getVM)

    # 4. Свойство позволяет только менять фамилию человека
    def __zamfio(self, newfio):
        self.fio = newfio

    v1 = property(None, __zamfio)
...
def isint(s):  # Проверка на целостность
    try:
        int(s)
        return True
    except ValueError:
...
def VM():  # 1. Вывести информацию по человеку, фун.
    global M
    TF = 1
    while TF == 1:
...
class Tstudent(Tpeople):  # Наследственный класс студент
    def __init__(self, fio, DR, pol, stud):  # Начальные настройки
        Tpeople.__init__(self, fio, DR, pol)  # Начальные настройки
        self.stud = stud

    def TT(self):
        return str(self.fio) + ' ' + str(self.VD()) + ' ' + str(self.pol)


class Tticher(Tpeople):  # Наследственный класс учитель
    def __init__(self, fio, DR, pol, chek):  # Начальные настройки
        Tpeople.__init__(self, fio, DR, pol)  # Начальные настройки
        self.chek = chek
...
while TT == 1:
    ...
    print('5. Вывод нужного человека;')
    print('6. Изменить фамилию человека;')
    print('7. Подкласс студент;')
    print('8. подкласс учитель;')
    ...
    if z == 1:  # Вывод информации по спискам
        ...
        i = int(input('Введите id человека: '))
        M[i].v1 = input('Введите изменённые ФИО')
    else:
        if z == 7:
            N = []
            N.append(Tstudent('dd', '02.04.3000', 'w', 123))  # 1 Создание объектов
            i = int(input('Введите id человека: '))
            N[i].zamena()
            print(N[i].TT())
        elif z == 8:
            N = []
            N.append(Tticher('dd', '02.04.3000', 'w', 124))  # 1 Создание объектов
            i = int(input('Введите id человека: '))
            N[i].zamena()
            print(N[i].TT())
